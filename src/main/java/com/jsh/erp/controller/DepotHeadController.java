...
//        }
    ...
    public BaseResponseInfo getBuyAndSaleStatistics(HttpServletRequest request) {
        ...
        try {
            ...
            String yesterday = Tools.getPreviousDate() + " 00:00:00";
            String firstDay = Tools.getCurrentMonth() + "-01 00:00:00";
            String previousMonthFirstDay = Tools.getPreviousMonth() + "-01 00:00:00";
            String lastWeek = Tools.getPreviousWeekDate() + " 00:00:00";
            String lastEndWeek = Tools.getPreviousLastWeekDate() + " 00:00:00";
            ...
            BigDecimal todayRetailSale = depotHeadService.getBuyAndSaleRetailStatistics("出库", "零售",
                    0, today, getNow3()); //今日零售出库金额

            BigDecimal yesterdayNum = depotHeadService.getBuyAndSaleTodayNum("出库", "零售",
                    0, yesterday, today); //作日销售出库件数
            BigDecimal yesterdayRetailSale = depotHeadService.getBuyAndSaleRetailStatistics("出库", "零售",
                    0, yesterday, today); //作日零售出库金额

            BigDecimal lastWeekDayNum = depotHeadService.getBuyAndSaleTodayNum("出库", "零售",
                    0, lastWeek, lastEndWeek); //上周今日销售出库件数
            BigDecimal lastWeekDaySale = depotHeadService.getBuyAndSaleRetailStatistics("出库", "零售",
                    0, lastWeek, lastEndWeek); //上周今日零售出库金额

            BigDecimal thisMonthSaleNum = depotHeadService.getBuyAndSaleTodayNum("出库", "零售",
                    0,firstDay, getNow3()); //本月零售出库数量
            BigDecimal monthRetailSale = depotHeadService.getBuyAndSaleRetailStatistics("出库", "零售",
                    0,firstDay, getNow3()); //本月零售出库金额

            BigDecimal previousMonthSaleNum = depotHeadService.getBuyAndSaleTodayNum("出库", "零售",
                    0,previousMonthFirstDay, firstDay); //上月零售出库数量
            BigDecimal previousMonthRetailSale = depotHeadService.getBuyAndSaleRetailStatistics("出库", "零售",
                    0,previousMonthFirstDay, firstDay); //上月零售出库金额


            BigDecimal todayBuyNum = depotHeadService.getBuyAndSaleTodayNum("入库", "采购",
                    1, today, getNow3()); //今日采购入库件数
            BigDecimal todayBuy = depotHeadService.getBuyAndSaleStatistics("入库", "采购",
                    1, today, getNow3()); //今日采购入库金额


            BigDecimal monthBuyNum = depotHeadService.getBuyAndSaleTodayNum("入库", "采购",
                    1, firstDay, getNow3()); //本月采购入库件数
            BigDecimal monthBuy = depotHeadService.getBuyAndSaleStatistics("入库", "采购",
                    1, firstDay, getNow3()); //本月采购入库金额
            map.put("todaySale", todayRetailSale);
            map.put("todayNum", todayNum);

            map.put("yesterdayNum", yesterdayNum);
            map.put("yesterdaySale", yesterdayRetailSale);

            map.put("thisMonthSale", monthRetailSale);
            map.put("thisMonthSaleNum", thisMonthSaleNum);

            map.put("lastWeekDayNum", lastWeekDayNum);
            map.put("lastWeekDaySale", lastWeekDaySale);

            map.put("todayBuyNum", todayBuyNum);
            map.put("todayBuy", todayBuy);

            map.put("thisMonthBuyNum", monthBuyNum);
            map.put("thisMonthBuy", monthBuy);

            map.put("previousMonthSaleNum", previousMonthSaleNum);
            map.put("previousMonthRetailSale", previousMonthRetailSale);
