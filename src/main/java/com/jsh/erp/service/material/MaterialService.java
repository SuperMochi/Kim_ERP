...
public class MaterialService {
    ...
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public BaseResponseInfo importExcelForsale(Sheet src) throws Exception {
        List<MaterialWithInitStock> mList = new ArrayList<>();
        ...
        //子单默认赋值
        depotItem.setMaterialUnit("件");
        depotItem.setMaterialExtendId(43L);
        depotItem.setDeleteFlag("0");
        depotItem.setDepotId(14L);
        double totalMoney = 0;
        for (int i = 1; i < src.getRows(); i++) {
            String orderNumber = ExcelUtils.getContent(src, i, 0);//订单号
            String dates = ExcelUtils.getContent(src, i, 2); //订单日期
            String model = ExcelUtils.getContent(src, i, 12); //类型编码
            String number1 = ExcelUtils.getContent(src, i, 14); //数量
            String money1 = ExcelUtils.getContent(src, i, 16); //价格
            double number = 0;
            Date operDate=new Date();
            if (StringUtil.isNotEmpty(number1)) {
                number = Double.parseDouble(number1);
                String[] splits = dates.split("\\+");
                String split = splits[0];
                DateFormat df = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
                Date date = df.parse(split);
                DateFormat df2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                String format = df2.format(date);
                operDate = df2.parse(format);
            }
            if (number != 0) {
                double money = Double.parseDouble(money1);
                totalMoney = number * money;  //该商品的总价
                depotHead.setDefaultNumber(orderNumber);
                depotHead.setNumber(orderNumber);
                depotHead.setChangeAmount(BigDecimal.valueOf(totalMoney));
                depotHead.setTotalPrice(BigDecimal.valueOf(totalMoney));
                depotHead.setOperTime(operDate);

                depotItem.setAllPrice(BigDecimal.valueOf(totalMoney));
                depotItem.setBasicNumber(BigDecimal.valueOf(number));
                depotItem.setOperNumber(BigDecimal.valueOf(number));
                depotItem.setUnitPrice(BigDecimal.valueOf(money));
                //根据商品编码查找商品id
                MaterialExample dhExample1 = new MaterialExample();
                dhExample1.createCriteria().andModelEqualTo(model).andDeleteFlagNotEqualTo(BusinessConstants.DELETE_FLAG_DELETED);
                List<Material> materials = materialMapper.selectByExample(dhExample1);
                if (materials.size() != 0) {
                    Long materalId = materials.get(0).getId();
                    depotItem.setMaterialId(materalId);
                } else {
                    throw new Exception("编码为：" + model + "商品未录入");
                }
                ...
                if (list.size() != 0) {
                    DepotHead depotHeadBefore = list.get(0);
                    Long headId = depotHeadBefore.getId();
                    depotItem.setHeaderId(headId);
                    BigDecimal add = BigDecimal.valueOf(totalMoney).add(depotHeadBefore.getTotalPrice());
                    depotHeadBefore.setTotalPrice(add);
                    depotHeadBefore.setChangeAmount(add);
                    depotHeadMapper.updateByPrimaryKey(depotHeadBefore);
                    depotItemMapper.insertSelective(depotItem);
                } else {
                    depotHeadMapper.insertSelective(depotHead);
                    DepotHeadExample dhExample2 = new DepotHeadExample();
                    dhExample2.createCriteria().andDefaultNumberEqualTo(depotHead.getDefaultNumber()).andDeleteFlagNotEqualTo(BusinessConstants.DELETE_FLAG_DELETED);
                    List<DepotHead> list2 = depotHeadMapper.selectByExample(dhExample2);
                    DepotHead depotHeadBefore = list2.get(0);
                    Long headId = depotHeadBefore.getId();
                    depotItem.setHeaderId(headId);
                    depotItemMapper.insertSelective(depotItem);
                }
...
//        for (int i = 2; i < src.getRows(); i++) {
//            String orderNumber = ExcelUtils.getContent(src, i, 0);//订单号
//            String model = ExcelUtils.getContent(src, i, 1); //类型编码
//            String number1 = ExcelUtils.getContent(src, i, 2); //数量
//            String money1 = ExcelUtils.getContent(src, i, 3); //价格
//            String dates = ExcelUtils.getContent(src, i, 4); //订单日期
//            double number = 0;
//            Date operDate=new Date();
//            if (StringUtil.isNotEmpty(number1)) {
//                number = Double.parseDouble(number1);
