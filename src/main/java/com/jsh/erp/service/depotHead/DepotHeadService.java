...
public class DepotHeadService {
     ...
     */
    @Transactional(value = "transactionManager", rollbackFor = Exception.class)
    public void addDepotHeadAndDetailDemo(String beanJson, String rows, Long tenantId,
                                      HttpServletRequest request) throws Exception {
        /**处理单据主表数据*/
        DepotHead depotHead = JSONObject.parseObject(beanJson, DepotHead.class);
        //判断用户是否已经登录过，登录过不再处理
        ...
        DepotHead depotHead1=new DepotHead();
        depotHead1.setType("出库");
        depotHead1.setSubType("零售");
        depotHead1.setPayType("现付");
        depotHead1.setAccountId(17L);
        depotHead1.setDefaultNumber(number);
        depotHead1.setNumber(number);
        depotHead1.setOperTime(new Timestamp(System.currentTimeMillis()));
        depotHead1.setCreator(userInfo==null?null:userInfo.getId());
        depotHead1.setCreateTime(new Timestamp(System.currentTimeMillis()));
        depotHead1.setStatus(BusinessConstants.BILLS_STATUS_UN_AUDIT);
        try{
            depotHeadMapper.insertSelective(depotHead);
        }catch(Exception e){
            JshException.writeFail(logger, e);
    ...
    public BigDecimal getBuyAndSaleTodayNum(String type, String subType, Integer hasSupplier, String beginTime, String endTime) {
        return depotHeadMapperEx.getBuyAndSaleNum(type, subType, hasSupplier, beginTime, endTime);
    }
